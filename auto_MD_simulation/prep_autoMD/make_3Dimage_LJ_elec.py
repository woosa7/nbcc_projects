...
class Image3D_LJ_elec:
    ...
    def make_feature(self):
        ...
        # protein.pql
        pql_file = '{}/calc/_3drism_1000.inp'.format(self.source_dir)
        ...
        col_names = ['atom_charge','LJ_sigma','LJ_epsilon','x','y','z']
        df_pql = pd.read_csv(pql_file, sep='\s+', skiprows=8, names=col_names)
        ...
        print('residues      :', residue_num, self.pdb_code)
        ...
        for i in range(data_num):                   # 데이터 (conformation) 수
                ...
                for box_i2 in range(other_number):
                    ...
                    if (x<=max and x>=min) and (y<=max and y>=min) and (z<=max and z>=min) :
                        ...
                        if other_residue[box_i2,1].find("C")==0:
                            OT_cnt+=1
                            Temp_box1[x,y,z] = other_residue[box_i2,6]  # atom_charge
                            Temp_box2[x,y,z] = other_residue[box_i2,7]  # LJ_sigma
                            Temp_box3[x,y,z] = other_residue[box_i2,8]  # LJ_epsilon
                        elif other_residue[box_i2,1].find("N")==0:
                            OT_cnt+=1
                            Temp_box1[x,y,z] = other_residue[box_i2,6]
                            Temp_box2[x,y,z] = other_residue[box_i2,7]
                            Temp_box3[x,y,z] = other_residue[box_i2,8]
