...
from attention_model import NBCC_RGN_Attention, adam_optimizer
...
if model_type == 1:
    ...
    model = NBCC_RGN_Attention_2(batch_size)
    ...
    for element in train_dataset:
        ...
        with tf.GradientTape() as tape:
            if model_type == 1:
                ...
                sos = tf.ones(shape=[batch_size, 1, 3], dtype=tf.float32)
                labels = tf.concat([sos, tertiaries], axis=1)
                shifted_labels = labels[:,:-1,:]

            # sos = tf.ones(shape=[1, batch_size, 9], dtype=tf.float32)
            # labels = tf.concat([sos, labels], axis=1)
            # shifted_labels = labels[:-1,:,:]
