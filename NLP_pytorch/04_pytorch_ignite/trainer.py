...
class MyEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        # Calculate accuracy only if 'y' is LongTensor, which means that 'y' is one-hot representation.
        # --> classification
        ...
        # l2 norm of parameters (학습이 진행될수록 값이 커져야 함)
        p_norm = float(get_parameter_norm(engine.model.parameters()))
        # l2 norm of gradient (학습 초기에는 값이 크며 점점 작아진다.) - 학습의 안전성 판단.
    ...
    def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                print('Epoch {} - |param|={:.3f} |g_param|={:.3f} loss={:.5f} accuracy={:.5f}'.format(
                    engine.state.epoch,
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_valid_logs(engine):
                print('Validation - loss={:.5f} accuracy={:.5f} best_loss={:.5f}'.format(
                    engine.state.metrics['loss'],
                    engine.state.metrics['accuracy'],
                    engine.best_loss,
                ))
