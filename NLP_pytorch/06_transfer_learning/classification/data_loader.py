...
def divide_dataset(train_set,
    ...
    if data_name == 'catdog':
        # test data가 라벨링이 되어 있지 않으므로 train data에서 test data 분리
...
def get_loaders(config, input_size):
    ...
    train_set = load_dataset(data_transforms, dataset_name=dataset_name, is_train=True)
    test_set  = load_dataset(data_transforms, dataset_name=dataset_name, is_train=False)
