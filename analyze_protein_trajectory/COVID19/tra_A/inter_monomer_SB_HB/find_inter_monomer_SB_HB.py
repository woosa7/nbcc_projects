...
"""
find inter-monomer SB and HB using the pre-defined cutoff value
...
with open('residue_info.dat', 'r') as f:
    ...
    for line in lines:
        ...
        if iend <= natom_monomer_1: # monomer_1
            res_m1.append(ires)
            start_m1.append(istart)
            end_m1.append(iend)
        else:                       # monomer_2
            res_m2.append(ires)
            start_m2.append(istart)
            end_m2.append(iend)
...
# find atom pairs making inter-monomer SB and HB
positive_res = ['ARG','LYS']
...
for k, file in enumerate(list_pdb):
    ...
    # collect data only for N or O atom
    data_m1 = []
    data_m2 = []
    str_atom = ''
    str_res = ''
    ...
    with open(file, 'r') as f:
        ...
        for line in lines:
            if line.startswith('ATOM'):
                ...
                if atom_name[0] == 'N' or atom_name[0] == 'O':
                    if res_no < res_m2[0]:
                        data_m1.append(atom)
                    else:
                        data_m2.append(atom)
    ...
    for i_atom in data_m1:
        ...
        for j_atom in data_m2:
            ...
            if dist2 <= cutoff2:
                ...
                # check if the contact is SB or HB
                if ((ires in negative_res) and (jres in positive_res)) or \
                   ((ires in positive_res) and (jres in negative_res)) :
                    ctype = 'SB'
                else:
                    ctype = 'HB'
                ...
                str = '{:>5d} {:<4} {:>3d} {} : {:>5d} {:<4} {:>3d} {} : {:8.3f} {}'.format(
                                    ...
                                    jnum, jatom, jresnum, jres, dist, ctype)
                ...
                if ipair_residues[iresnum,jresnum] == 0:
                    ...
                    # SB_HB_residues
                    str = '{:>3d} {} : {:>3d} {} {}'.format(iresnum, ires, jresnum, jres, ctype)
                    str_res = str_res + str + '\n'


    f_atom = open('SB_HB_atoms.dat', 'w')
    print('{:>12d}'.format(n_SB_HB_atoms), file=f_atom)
    print(str_atom, file=f_atom)
    f_atom.close()

    f_res = open('SB_HB_residues.dat', 'w')
    print('{:>12d}'.format(n_SB_HB_residues), file=f_res)
    print(str_res, file=f_res)
    f_res.close()

    print('{} : atom {} : res {}'.format(frame_dir, n_SB_HB_atoms, n_SB_HB_residues))
